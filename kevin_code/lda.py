...
# Run LDA
for topic_num in no_topics:
    lda = LatentDirichletAllocation(n_topics=topic_num, max_iter=5, learning_method='online', learning_offset=50.,random_state=0).fit_transform(tf)
    ...
    np.save('LDA_KL_Divergence_' + str(topic_num) + '_topics', lda_kl_divergence)
    write_csv('LDA_KL_Divergence' + str(topic_num) + '_topics', lda_kl_divergence)
