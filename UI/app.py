...
def results(course_name,lno,curr_slide):
    search = forms.SearchForm(request.form)
    #if request.method == 'POST':
    return search_results(course_name,lno,curr_slide, search)
 
    #return render_template('search.html', form=search)
 
 
@app.route('/results')
def search_results(course_name,lno,curr_slide, search):
    results = []
    ...
    curr_slide, lno,lec_name,(num_results,results,disp_strs,course_names,lnos),lec_names,lnos = model.get_search_results(search_string)
    print(num_results)
    if not results:
        return render_template("base.html",course_names=COURSE_NAMES,num_courses=NUM_COURSES)
    else:
        # display results
        return render_template("search.html",slide_name=curr_slide,course_name=course_name,num_results=num_results,results = results,disp_str=disp_str,related_course_names=related_course_names,lno=lno,lec_name=lec_name,lec_names=lec_names,lnos=lnos,course_names=COURSE_NAMES,num_courses=NUM_COURSES,rel_lnos=rel_lnos)
