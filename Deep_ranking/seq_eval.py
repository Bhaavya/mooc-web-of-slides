...
def is_seq(slide1_name,slide2_name,m=2):
...
def get_seq_slides_all(slide_names,all_slide_names,m=2):
	all_seq_slides = []
	for i,slide_name in enumerate(slide_names):
		all_seq_slides.append(get_seq_slides_single(slide_name,all_slide_names,m))
...
def calc_recall_k(sim_mat,all_seq_slides,subtitles_lst,slide_info_lst,slide_names,k=20,analysis_file_prefix=None):
	...
	for i,sim_slides in enumerate(most_sim):
		...
		if is_actual_slide(i,slide_names,slide_info_lst):
			if(len(all_seq_slides[i])>0):
				actual_slides+=1
				k_most_sim_mat[i] = [np.nan for s in range(k)]
				sim_slide_num = 0
				for j in sim_slides:
					if not sim_slide_num==k:
						if is_actual_slide(j,slide_names,slide_info_lst):
	...
	if analysis_file_prefix is not None:
		analyze_results(np.array(k_most_sim_mat),sim_mat,all_seq_slides,analysis_file_prefix,slide_names,slide_info_lst)

def analyze_results(k_most_sim,sim_mat,all_seq_slides,analysis_file_prefix,slide_names,slide_info_lst):
	failures = []
	winners = []
	print (k_most_sim.shape)
	for i,sim_slides in enumerate(k_most_sim):
		if sim_slides!=[]:
			curr_slide_info = slide_info_lst[i]
			for j in sim_slides:
				sim_slide_info = slide_info_lst[j]
				info = {'name':slide_names[i],'k_most_sim':slide_names[j],'similarity':sim_mat[i,j]}
...
def analyze_results2(k_most_sim,sim_mat,all_seq_slides,analysis_file_prefix,slide_names,slide_info_lst):
	failures = {}
	winners = {}
	print (k_most_sim.shape)
	for i,sim_slides in enumerate(k_most_sim):
		if len(sim_slides)!=0:
			slide_dict = {}
			slide_dict[slide_names[i]] = []
			for j in sim_slides:
					slide_dict[slide_names[i]].append(slide_names[j])
			if len(set(all_seq_slides[i]).intersection(sim_slides)) == 0:
	...
	with open(analysis_file_prefix+'_winners_names.txt', 'w') as output_file:
		for k in winners:
			output_file.write(k)
	with open(analysis_file_prefix+'_failures_names.txt', 'w') as output_file:
		for k in failures:
			output_file.write(k)
...
def main(sim_mat_file,slide_names_file,subtitles_file,format,correct_names,analysis_file_prefix=None,courses_json=None):
	...
	print ('Number of slide names:',len(slide_names))
	...
	slide_info_lst = get_slide_info(courses_json,slide_names)
	
	calc_recall_k(sim_mat,all_seq_slides,subtitles_lst,slide_info_lst,slide_names,analysis_file_prefix=analysis_file_prefix)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--names', type=str, help='path to slides names order')
    ...
    main(args.mat,args.names,args.subtitles,args.format,args.correct_names,args.analysis_file_prefix,args.courses_json)
